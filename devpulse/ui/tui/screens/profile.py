"""Profile screen — developer fingerprint with energy/workflow/focus."""
...
def _hourly_sparkline(hourly: list[dict]) -> str:
    """Build a 24-char sparkline from hourly command counts."""
    if not hourly:
        return ""
    counts = [h.get("commands", 0) for h in hourly]
    max_v = max(counts) or 1
    return "".join(
        _HOUR_BLOCKS[min(7, int(c / max_v * 7))] for c in counts
    )
...
class ProfileScreen(VimVerticalScroll):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("ENERGY MAP — commands by hour (0-23)", classes="section-bar")
        yield Static("", id="prof-energy", classes="muted")

        yield Static("PEAK HOURS", classes="section-bar")
        yield Static("", id="prof-peak", classes="muted")

        yield Static("WORKFLOW FINGERPRINT", classes="section-bar")
        yield Static("", id="prof-workflow", classes="muted")

        yield Static("FOCUS PATTERN", classes="section-bar")
        yield Static("", id="prof-focus", classes="muted")

        yield Static("[dim]Press [R] to regenerate the profile (slower)[/dim]", classes="muted")

    async def refresh_data(self) -> None:
        self.query_one("#prof-title", Static).update(
            "\n[bold]Developer Profile[/bold]  [dim]your unique fingerprint[/dim]\n"
        )
    ...
    def _render_profile(self, result: dict) -> None:
        ...
        if len(cards) >= 4:
            ...
            trend_color = "#27a644" if trend == "improving" else "#e87b5a" if trend == "worsening" else "#fbbf24"
        ...
        # Energy map
        spark = _hourly_sparkline(energy.get("hourly", []))
        hour_labels = "0    4    8    12   16   20   "
        self.query_one("#prof-energy", Static).update(
            f" [#5e6ad2]{spark}[/#5e6ad2]\n"
            f" [dim]{hour_labels}[/dim]\n"
            f" [dim]Total commands: {energy.get('total_commands', 0)} · Total commits: {energy.get('total_commits', 0)}[/dim]"
        )
        ...
        self.query_one("#prof-peak", Static).update(
            f" [#27a644]●[/#27a644] [bold]Peak:[/bold] {peak_str}  [dim](highest commit/error ratio)[/dim]\n"
            f" [#e87b5a]●[/#e87b5a] [bold]Low energy:[/bold] {low_str}  [dim](>20% error rate)[/dim]"
        )

        # Workflow
        tools = workflow.get("tools_top_5", [])
        wf_lines = []
        wf_lines.append(f" [bold]Top tools:[/bold] {' · '.join(tools) if tools else '—'}")
        wf_lines.append(
        ...
        fc_lines.append(
            f" [bold]Top distractors:[/bold] {', '.join(distractors) if distractors else '—'}"
        )
